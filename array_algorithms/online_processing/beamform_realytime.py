...
array3 = pra.Beamformer(R3, Fs, nfft)
...
ax.set_title("Beamformed")

lines = []
lines.append(ax.plot(plotdata))
print(lines)

array3.rake_max_sinr_weights(
		source = pra.soundsource.SoundSource([5,5]),
		interferer = pra.soundsource.SoundSource([-5,5])
    )
...
def audio_callback(indata,frames,time,status):
	...
	# global block_number, ks, previous_az, array3
	# # normalize data
	...
	# 	block_number += 1
	# else:
	# 	data_array = np.array([ks[i]*data_array[i,:]for i in range(channels)])
	# array3.signals = data_array[[7,6,4,1,0],:]
	# print(array3.signals.shape)
	
	# output3 = array3.process()
	# print(output3.shape)
	# out_norm = pra.normalize(output3)
	# print(out_norm.shape)

	# return_data = out_norm[0:-1023]
	out_norm = np.random.rand(blocksize)
	# print(return_data)
	# print(return_data.shape)
	# return_data = np.zeros(blocksize)
